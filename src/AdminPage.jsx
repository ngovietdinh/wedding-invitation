...
const DEF = {
  ...
  love_story:[
    ...
    {date:"03.2022",title:"Hẹn hò chính thức",body:"Dưới ánh hoàng hôn Huế, anh nắm tay em và hỏi một câu quan trọng.",emoji:"🌹"},
    {date:"06.2022",title:"Chuyến đi đầu tiên",body:"Cùng nhau khám phá thành phố mới, mỗi bước chân đều là kỷ niệm.",emoji:"🗺️"},
    {date:"12.2023",title:"Vượt qua thử thách",body:"Có những lúc khó khăn nhưng chúng ta luôn chọn ở bên nhau.",emoji:"🤝"},
    {date:"02.2025",title:"Lời cầu hôn",body:"Trong khoảnh khắc đặc biệt, anh quỳ xuống và hỏi: Em có muốn làm vợ anh không?",emoji:"💍"},
    {date:"04.2026",title:"Ngày trọng đại",body:"Hành trình tình yêu dẫn chúng ta đến đây — cùng nhau bắt đầu chương mới.",emoji:"💒"},
  ],
};
...
.flip-clock{
  ...
  gap:4px;padding:18px 12px 12px;
}
.flip-unit{display:flex;flex-direction:column;align-items:center;gap:5px;min-width:58px;}
.flip-cards{
  position:relative;width:58px;height:72px;
  perspective:300px;
}
/* Card bóng nền */
.flip-shadow{
  position:absolute;bottom:-4px;left:6px;right:6px;height:8px;
  background:rgba(0,0,0,.3);filter:blur(4px);border-radius:50%;
}
/* Top half */
.flip-top{
  position:absolute;top:0;left:0;right:0;height:36px;
  background:linear-gradient(180deg,#5a1020 0%,#4a0e18 100%);
...
.flip-bot-static{
  ...
  background:linear-gradient(180deg,#3a0810 0%,#2d0a12 100%);
  border-radius:0 0 6px 6px;
  display:flex;align-items:flex-start;justify-content:center;
  padding-top:2px;overflow:hidden;z-index:1;
}
/* Flap đang lật */
.flip-flap{
  position:absolute;top:0;left:0;right:0;height:36px;
...
.flip-flap.go{animation:flapAnim .32s ease-in forwards;}
@keyframes flapAnim{
...
.flip-flap2.go{animation:flapAnim2 .32s ease-out .32s forwards;}
@keyframes flapAnim2{
...
.flip-digit.bot-num{
  transform:translateY(-36px);/* hiện phần dưới */
}
/* Đường kẻ giữa */
.flip-line{
  position:absolute;top:35px;left:0;right:0;height:2px;
  background:rgba(0,0,0,.5);z-index:4;
}
/* Separator */
.flip-sep{
  font-size:24px;font-weight:700;
  color:rgba(255,150,130,.5);
  line-height:1;margin-bottom:14px;
  font-family:'Cinzel',serif;
  animation:sepBlink 1s ease-in-out infinite;
}
@keyframes sepBlink{0%,100%{opacity:.5;}50%{opacity:1;}}
/* Label */
.flip-label{
  font-size:8px;letter-spacing:.22em;text-transform:uppercase;
  color:rgba(255,175,155,.6);font-family:'Quicksand',sans-serif;
  font-weight:600;
}
...
// ══════════════════════════════════════════════════════
// FLIP CLOCK — 3D đồng hồ lật chuẩn
// ══════════════════════════════════════════════════════
function FlipDigit({ value }) {
  ...
  useEffect(() => {
    ...
    const p = String(prevVal.current).padStart(2,"0");
    if (v === p) return;
    // Bắt đầu animation lật
    setNext(v);
    setFlipping(true);
    setAnimKey(k => k+1);
    const t = setTimeout(() => {
  ...
  return (
    <div className="flip-cards">
      ...
      {/* Nửa trên tĩnh — số hiện tại */}
      <div className="flip-top">
        <span className="flip-digit">{cur}</span>
      </div>
      {/* Nửa dưới tĩnh — số mới */}
      <div className="flip-bot-static">
        <span className="flip-digit bot-num">{next}</span>
      </div>
      {/* Flap trên đang lật xuống (overlay top) */}
      {flipping && (
        <div key={`t-${animKey}`} className="flip-flap go">
          <span className="flip-digit">{cur}</span>
        </div>
      )}
      {/* Flap dưới đang lật lên (overlay bot) */}
      {flipping && (
        <div key={`b-${animKey}`} className="flip-flap2 go">
          <span className="flip-digit bot-num">{next}</span>
        </div>
      )}
      {/* Đường kẻ giữa */}
  ...
  if (cd.past) return (
    <div style={{textAlign:"center",padding:"1.5rem 0"}}>
      <p style={{fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",
        fontSize:"22px",color:"rgba(255,210,200,.92)"}}>
