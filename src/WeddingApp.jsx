...
const DEF = {
  ...
  // object-position cho từng ảnh (dùng để căn chỉnh)
  hero_pos:"center center",couple_pos:"center center",
  large_pos:"center center",sm1_pos:"center center",
  sm2_pos:"center center",wide1_pos:"center center",
...
/* ── Music button ── */
#aud{
  position:fixed;right:calc(50% - 225px + 8px);top:calc(3vh + 8px);
  z-index:9999;width:38px;height:38px;
  background:rgba(60,10,10,.55);
  border-radius:50%;border:2px solid rgba(240,180,180,.5);
  display:flex;align-items:center;justify-content:center;
  cursor:pointer;backdrop-filter:blur(8px);
  transition:all .3s;
}
#aud:hover{background:rgba(99,23,23,.75);border-color:rgba(240,180,180,.8);}
#aud.on{
  background:linear-gradient(135deg,#631717,#9a2a2a);
  border-color:rgba(255,200,200,.6);
  box-shadow:0 0 12px rgba(99,23,23,.6);
...
/* ── Falling particles ── */
.particle{position:fixed;top:-40px;pointer-events:none;z-index:9997;
...
/* ── Calendar ── */
.cal-d{display:flex;align-items:center;justify-content:center;aspect-ratio:1;
  border-radius:50%;font-size:12px;color:#444;font-family:'Quicksand',sans-serif;font-weight:500;}
.cal-d.sp{background:linear-gradient(135deg,#631717,#9a2a2a);color:#fff;font-weight:700;
  font-size:13px;position:relative;animation:pGlow 2.5s ease-in-out infinite;}
.cal-d.sp::before{content:'♥';position:absolute;top:-13px;left:50%;
  transform:translateX(-50%);font-size:10px;color:#9a2a2a;}
.cal-d.wk{color:#4a4a8a;}

/* ── RSVP ── */
.rv-in{width:100%;padding:7px 9px;border:1px solid #d4b8b8;border-radius:4px;
  font-size:12px;margin-bottom:10px;background:#fdf7f7;color:#333;
  font-family:'Quicksand',sans-serif;transition:border-color .22s;outline:none;}
.rv-in:focus{border-color:#631717;}
.rv-rb label{display:flex;align-items:center;gap:7px;font-size:12px;color:#444;
  font-family:'Quicksand',sans-serif;cursor:pointer;margin-bottom:6px;}
.rv-rb input{accent-color:#631717;}

/* ── Lightbox ── */
#lb{display:none;position:fixed;inset:0;z-index:99999;
...
#lb-cl{position:absolute;top:.8rem;right:.8rem;background:transparent;
  border:1px solid rgba(255,255,255,.35);color:rgba(255,255,255,.7);
  font-size:.58rem;letter-spacing:.28em;text-transform:uppercase;
  padding:.35rem .8rem;cursor:pointer;font-family:'Quicksand',sans-serif;}
#lb-pv,#lb-nx{position:absolute;top:50%;transform:translateY(-50%);
  background:rgba(99,23,23,.25);border:1px solid rgba(240,180,180,.35);
  color:rgba(255,220,220,.85);font-size:1.8rem;
  width:42px;height:64px;cursor:pointer;font-family:serif;
  display:flex;align-items:center;justify-content:center;border-radius:4px;}
#lb-pv{left:.8rem;}#lb-nx{right:.8rem;}
#lb-cap{position:absolute;bottom:1rem;left:50%;transform:translateX(-50%);
...
/* ── Photo với bo cong nghệ thuật ── */
.photo-frame{
  position:relative;overflow:hidden;
  /* Bo cong nghệ thuật — có thể tuỳ chỉnh */
}
.photo-frame.round-art{
  border-radius:40% 60% 55% 45% / 45% 40% 60% 55%;
}
.photo-frame.round-soft{
  border-radius:20px;
}
.photo-frame.round-circle{
  border-radius:50%;
}
.photo-frame.round-wave{
  border-radius:62% 38% 46% 54% / 60% 44% 56% 40%;
}
.photo-frame img,.photo-frame .photo-bg{
  width:100%;height:100%;object-fit:cover;display:block;
}
.photo-frame .photo-overlay{
  position:absolute;inset:0;
  border:2px solid rgba(99,23,23,.2);
...
@media(max-width:460px){
  ...
  #pw{width:100vw;max-height:none!important;height:auto!important;
...
function useCd(target) {
  ...
  useEffect(()=>{
    const c=()=>{const d=new Date(target)-new Date();if(d<=0)return;
...
// ══════════════════════════════════════════════
// MUSIC — Auto-play trên mobile
// Kỹ thuật: dùng Web Audio API + YouTube IFrame API
// Mobile yêu cầu user gesture để phát audio
// → phát ngay khi user chạm/click lần đầu
// ══════════════════════════════════════════════
function Music({url}) {
  ...
  // Load iframe ngay khi có URL (muted trước để vượt autoplay policy)
  // Sau đó unmute khi user tap
  const ifrSrc = id
    ? `https://www.youtube.com/embed/${id}?autoplay=1&mute=1&loop=1&playlist=${id}&controls=0&enablejsapi=1&playsinline=1`
    : "";

  // Khi iframe load xong
  const onIfrLoad = () => { setL(true); };

  // Hàm play thật sự (cần user gesture trên mobile)
  ...
  // Auto-play: phát ngay khi user chạm màn hình lần đầu
  useEffect(() => {
    if (!id) return;
    const tryPlay = () => {
      if (startedRef.current) return;
      startedRef.current = true;
      // Phải đợi iframe load
      setTimeout(doPlay, 800);
    };
    // Lắng nghe mọi gesture của user
    window.addEventListener("touchstart", tryPlay, { once: true, passive: true });
    window.addEventListener("click",      tryPlay, { once: true });
    // Desktop: thử sau 1.5s không cần gesture
    const t = setTimeout(() => {
  ...
  return (
    <>
      {/* Iframe luôn có mặt (muted) để vượt autoplay policy */}
      {id && (
        <iframe
          ref={ifrRef}
          src={ifrSrc}
      ...
      {/* Nút nhạc — icon NỐT NHẠC */}
      <button id="aud" className={on?"on":""} onClick={toggle}
        title={on?"Tắt nhạc":"Bật nhạc"}
        aria-label={on?"Tắt nhạc":"Bật nhạc"}>
        {on ? (
          /* Nốt nhạc đang phát — animated bars */
  ...
  return(
    <span ref={ref} style={{display:"inline-flex",flexWrap:"nowrap",whiteSpace:"pre",...style}}
      ...
      {chars.map((ch,i)=>{
        ...
        return <span key={i} className={`sc ${isL?"sl":"sr"}`}
...
function Rv({children,dir="u",delay=0,style={},className="",tag:Tag="div"}) {
  ...
  return(
    <Tag ref={ref} className={className}
      style={{opacity:vis?1:0,transform:vis?"none":(dirMap[dir]||"translateY(48px)"),
...
// ══════════════════════════════════════════════
// PHOTO COMPONENT — hỗ trợ URL, Google Drive, upload local
// Bo cong nghệ thuật + object-position tuỳ chỉnh
// ══════════════════════════════════════════════
function Photo({url, pos="center center", shape="soft", style={}, className="", onClick}) {
  const src = gd(url);
  const shapeClass = {
    soft:   "round-soft",
    art:    "round-art",
  ...
  if (!src) {
    return (
      <div className={`photo-frame ${shapeClass} ${className}`}
        style={{backgroundColor:"#c4a0a0",display:"flex",alignItems:"center",
          justifyContent:"center",...style}} onClick={onClick}>
        <span style={{fontSize:"1.5rem",opacity:.4}}>🖼</span>
      </div>
    );
  }
  return (
    <div className={`photo-frame ${shapeClass} ${className}`} style={style} onClick={onClick}>
...
function Fl({top,left,w,h,rot,op=0.2}) {
  return <div style={{position:"absolute",top,left,width:w,height:h,
    ...
    filter:"hue-rotate(0deg) saturate(.5) sepia(.3) brightness(1.1)",pointerEvents:"none",zIndex:0}}/>;
}

// Quote block với nền mờ đọc được trên ảnh
function QB({text,fontSize=17,style={}}) {
  return(
    <div style={{background:"rgba(0,0,0,.55)",backdropFilter:"blur(3px)",borderRadius:"6px",padding:"10px 14px",...style}}>
      <p className="on-photo" style={{fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",
        color:"rgba(255,230,230,.97)",textAlign:"center",lineHeight:1.65,fontSize,whiteSpace:"pre-line"}}>
        {text}
      </p>
    </div>
  );
...
export default function WeddingApp() {
  ...
  useEffect(()=>{
    ...
    const scrollBy_=(n)=>{if(isMobile())window.scrollBy(0,n);else pw.scrollTop+=n;};
    const getScrollTop_=()=>isMobile()?window.scrollY:pw.scrollTop;
    const getMax_=()=>isMobile()?document.documentElement.scrollHeight-window.innerHeight:pw.scrollHeight-pw.clientHeight;
    ...
    const loop=()=>{
      ...
      if(!paused){
        ...
        scrollBy_(SPEED);
        progScroll=false;
        if(getScrollTop_()>=getMax_()-2){running=false;return;}
      }
  ...
  // Trigger reveal đầu trang
  useEffect(()=>{
    const t=setTimeout(()=>{
      const vh=window.innerHeight;
      document.querySelectorAll(".rv:not(.show)").forEach(el=>{
        if(el.getBoundingClientRect().top<vh*1.05)el.classList.add("show");
      });
    },250);
  ...
  return(<>
    ...
    <div id="pw">
      ...
      {/* ═══ S1: COVER HERO ═══ */}
      <div style={{position:"relative",width:"100%",height:"490px",overflow:"hidden",
        background:"linear-gradient(148deg,#3d0e0e 0%,#631717 50%,#2a0808 100%)"}}>
        ...
        {/* Deco */}
        <Rv dir="r" delay={0.15} style={{position:"absolute",top:24,left:0,width:"130px",height:"165px",
          backgroundImage:"url('https://assets.cinelove.me/templates/assets/efd815e3-41ff-4eb3-b31b-c25b202bc08c/1faee750-3c82-4fdb-badb-f258477bd1c4.png')",
          backgroundSize:"cover",opacity:.65,filter:"hue-rotate(0deg) saturate(.4) sepia(.4) brightness(1.2)",zIndex:2}}/>

        <Rv dir="u" delay={0} style={{position:"absolute",top:10,left:0,width:"100%",textAlign:"center",zIndex:3}}>
          <p style={{color:"rgba(255,210,210,.82)",fontSize:"11.5px",fontWeight:600,
            fontFamily:"'Quicksand',sans-serif",letterSpacing:".5em",textTransform:"uppercase"}}>
            SAVE THE DATE
          </p>
        </Rv>

        {/* Panel phải */}
        <Rv dir="l" delay={0.12} style={{position:"absolute",top:75,right:6,width:"162px",
          ...
          <p style={{color:"rgba(255,210,210,.68)",fontSize:"9.5px",fontWeight:600,letterSpacing:".25em",textTransform:"uppercase",fontFamily:"'Quicksand',sans-serif"}}>{d.wedding_day}</p>
          <p style={{color:"#f0c0c0",fontSize:"13.5px",fontWeight:600,fontFamily:"'Cinzel',serif",letterSpacing:".13em"}}>{d.wedding_date}</p>
      ...
      <div style={{position:"relative",background:"#fff"}}>
        ...
        <Rv dir="u" delay={0}>
          <div style={{background:"linear-gradient(90deg,#3d0e0e,#631717,#3d0e0e)",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <p style={{fontSize:"21px",fontFamily:"'Cinzel',serif",fontWeight:600,color:"#fde8e8",letterSpacing:".07em"}}>Tháng 4</p>
            <p style={{fontSize:"12px",fontFamily:"'Quicksand',sans-serif",fontWeight:600,letterSpacing:".2em",color:"rgba(253,232,232,.72)"}}>2026</p>
          </div>
        </Rv>
        <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"0"}}>
          <Rv dir="s" delay={0.1} style={{overflow:"hidden"}}><Cal day={26}/></Rv>
          <Rv dir="l" delay={0.15}>
            ...
            {galArr.map((img,i)=>(
              <Rv key={i} dir="s" delay={i*.06}
                style={{overflow:"hidden",cursor:"pointer",
                  borderRadius:"12px",boxShadow:"0 4px 16px rgba(0,0,0,.45)"}}
                ...
                <img src={gd(img.url)} alt={img.caption||""} loading="lazy"
                  style={{width:"100%",height:"165px",objectFit:"cover",display:"block",
                    ...
                    transition:"transform .6s ease",borderRadius:"12px"}}
      ...
      <div style={{padding:"32px 28px",textAlign:"center",position:"relative"}} className="sec-night">
        ...
        <Rv dir="u" delay={0.1} style={{position:"relative",zIndex:1}}>
          <p style={{fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",fontSize:"22px",
            color:"rgba(255,220,220,.92)",lineHeight:1.65,whiteSpace:"pre-line",
      ...
      <div style={{background:"#110404",padding:"14px",textAlign:"center"}}>
        <p style={{color:"rgba(255,200,200,.3)",fontSize:"9.5px",fontFamily:"'Quicksand',sans-serif",letterSpacing:".22em"}}>
